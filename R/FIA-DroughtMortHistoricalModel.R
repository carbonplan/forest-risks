...
#--------------------------- Forest risk mapping - Fig 1C/D (all-mort historical) code - 03/30/21 -----------------------------------
...
FTpred_drt1 <- read.csv(paste(dir, "Drt_ForTypeToPredict_03-29-2021.csv", sep = ""), header = T)
FTpred2 <- FTpred_drt1[1:53, 2:5]
...
drought_histmodel <- function(fitdata, minthres, ftypcol) {
  ...
  for (i in 1:112) {
    ...
    drt.out[, 7] <- d1[, 13] # Summed BAlive0 for the FORTYP

    # If FORTYP has non-meaningful historical model, set future projections == mean historical
    if (ftype %!in% FTpred2[, 1]) drt.out[, 6] <- mean(d1[, 1], na.rm = T) # Predicted mort = mean historical where Nmort<minthres
    if (ftype %!in% FTpred2[, 1]) mod1 <- rbind(mod1, drt.out)
    if (ftype %!in% FTpred2[, 1]) print(paste("FORTYP prediction skipped due inadequate historical model: ", ftype, sep = ""))
    if (ftype %!in% FTpred2[, 1]) next
...
drought_predmodel <- function(fitdata, minthres, ftypcol, preddata) {
  ...
  for (i in 1:112) {
    ...
    non_zero <- ifelse(fitdata[which(fitdata[, ftypcol] == ftype), 1] > 0, 1, 0)
    d1 <- data.frame(fitdata[which(fitdata[, ftypcol] == ftype), ], non_zero)
    # NEW in v18: outlier screen + mean-historical-mort where not enough Nmort obs
...
prepclim <- function(climdata, histcols, futcols) {
  ...
  fialong[which(fialong[, 25] < -16), 25] <- -16
  fialong[which(fialong[, 25] > 16), 25] <- 16
  fialong[which(fialong[, 26] < -16), 26] <- -16
  fialong[which(fialong[, 26] > 16), 26] <- 16
...
#--------------------------------------- Run model for Fig 1C/D
histcols1 <- c(20, 24, 25, 30, 32, 34)
futcols1 <- c(4, 17, 12, 19, 8, 15)
drought.outlong <- drought_predmodel(fia.g1[, c(1, 3, 7, 11, 13, 15, 20, 22, 24, 29)], 21, 10, prepclim(fialong, histcols1, histcols1))
drought.out <- drought_histmodel(fia.g1[, c(1, 3, 7, 11, 13, 15, 20, 22, 24, 29, 28, 27, 30)], 21, 10)

# Scale up
mort.drought.obs <- weight.proj(drought.out, dummy, 4)
mort.drought.mod <- weight.proj(drought.out, dummy, 6)
mort.drought.modlong <- weight.proj(drought.outlong, dummy, 6)

writeRaster(mort.drought.obs, paste(dir, "Fig1C_DroughtModel_ObservedHistMort_3-30-2021", sep = ""), format = "GTiff")
# writeRaster(mort.insects.mod, paste(dir, "Fig1F_InsectModel_ModeledHistMort_3-26-2021", sep=""),  format='GTiff')
writeRaster(mort.drought.modlong, paste(dir, "Fig1D_DroughtModel_ModeledFIAlongHistMort_3-30-2021", sep = ""), format = "GTiff")
