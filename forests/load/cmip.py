...
def subset_time_training(ds):
    return ds.sel(time=slice("1960", "2014"))
...
def cmip():
    ...
    data.update(
        hist_subset.to_dataset_dict(zarr_kwargs=zarr_kwargs, preprocess=preprocess_hist)
    )
    data.update(
        ssp_subset.to_dataset_dict(zarr_kwargs=zarr_kwargs, preprocess=preprocess_ssp)
    )
...
def sample_data():
    ...
    store = zarr.storage.ABSStore(
        "carbonplan-data",
        prefix=f"carbonplan-scratch/downscaling-point-data/terraclimate",
        account_name="carbonplan",
        account_key=os.environ["BLOB_ACCOUNT_KEY"],
    )
